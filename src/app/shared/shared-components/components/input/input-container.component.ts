...
import { InputContainerDefault } from "./model/input-container-const.model";
...
export class InputContainerComponent
    ...
    ngAfterContentInit() {
        if (this.input === undefined) {
            throw new Error(
                "Esse componente precisa ser utlizado com uma diretiva formControlName"
            );
        }
        ...
        if (msg != null) {
            switch (validatorName) {
                case "minlength":
                    return msg.replace(
                        "{VALUE}",
                        this.input.errors.minlength.requiredLength ||
                            "NOT_CONTENT"
                    );
                case "maxlength":
                    return msg.replace(
                        "{VALUE}",
                        this.input.errors.maxlength.requiredLength ||
                            "NOT_CONTENT"
                    );
                default:
                    return msg;
